...
export default function Discovery() {
  ...
  const isSaved = heroMovie ? isInWatchlist(heroMovie.id) : false;
            ...
            <div className="absolute inset-0 p-8 md:p-16 flex flex-col justify-center z-10">
              <div className="max-w-2xl space-y-6">
                ...
                <div className="hero-buttons flex items-center gap-4 pt-4">
                  ...
                  <Button variant="secondary" size="lg" className="gap-2" onClick={() => toggleWatchlist({...heroMovie, media_type: 'movie'})}>
                    {isSaved ? <Check className="w-5 h-5" /> : <Plus className="w-5 h-5" />}
                    {isSaved ? 'Added' : 'Add to Watchlist'}
                  </Button>
                  <Button variant="ghost" size="icon" className="rounded-full border border-white/10" onClick={() => navigate(`/movie/${heroMovie.id}`)}>
      ...
      <section className="space-y-6 gsap-trending-section">
        ...
        <div ref={carouselRef} className="flex gap-4 md:gap-6 overflow-x-auto pb-4 scrollbar-hide snap-x">
          {trendingLoading ? Array(6).fill(0).map((_,i) => <div key={i} className="shrink-0 w-32 md:w-40 animate-pulse aspect-[2/3] bg-white/5 rounded-xl" />) : 
            trending?.results?.slice(1, 10).map((movie: any) => (
            <Link to={`/movie/${movie.id}`} key={movie.id} className="group block relative shrink-0 w-32 md:w-40 snap-start">
              <div className="rounded-xl overflow-hidden glass-panel group-hover:-translate-y-2 group-hover:shadow-[0_10px_30px_rgba(168,85,247,0.2)] transition-all duration-300">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-obsidian via-obsidian/20 to-transparent p-4 flex flex-col justify-end">
                  <h3 className="font-sans font-semibold text-sm text-white truncate">{movie.title}</h3>
                  <div className="flex items-center justify-between mt-2">
                    <RatingBadge score={movie.vote_average} showText={false} />
        ...
        <section className="lg:col-span-2 space-y-6 gsap-box-office">
          <div className="flex items-center justify-between">
            <div>
              ...
              <h2 className="font-syne text-3xl font-bold text-white mt-1">Weekly Box Office Leaders</h2>
            </div>
            <span className="text-xs text-gray-500 font-mono hidden sm:block">Realtime Global + Domestic</span>
          </div>
            ...
            {popularLoading ? Array(4).fill(0).map((_,i) => <div key={i} className="animate-pulse h-24 bg-white/5 rounded-xl gsap-item" />) :
              ...
              <div key={movie.id} className="flex items-center gap-4 group p-2 hover:bg-white/5 rounded-xl transition-colors gsap-item">
                ...
                <div className="text-right">
                  <div className="font-sans font-bold text-sm md:text-base text-white">${(142.6 - idx * 22.4).toFixed(1)}M</div>
                  <div className="text-[10px] text-green-400 mt-1">+{(22.4 - idx * 3).toFixed(1)}% vs Prev Week</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <section className="space-y-6 gsap-critic">
          ...
          <div className="glass-panel p-6 rounded-xl border border-primary/20 relative">
            <div className="absolute -top-3 -right-3 bg-primary w-8 h-8 rounded-full flex items-center justify-center shadow-[0_0_15px_rgba(168,85,247,0.5)]">
              <span className="text-white text-xs font-bold">96%</span>
            </div>
            
            <div className="flex items-center gap-1 mb-4 text-accent-amber">
              {'★★★★★'.split('').map((star, i) => <span key={i} className="text-sm">{star}</span>)}
              <Badge variant="highlight" className="ml-auto">PREMIERE PICK</Badge>
            </div>
            
            <blockquote className="font-sans text-gray-200 text-sm md:text-base italic leading-relaxed">
              "An atmospheric triumph that blends neo-noir sensibilities with staggering cosmic scale. A cinematic achievement that defines the decade."
            </blockquote>
            ...
            <div className="mt-6 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-[10px] text-gray-500">Chief Cinema Critic, Culture Observ.</div>
                </div>
              </div>
            </div>
          </div>
